program low_pass_filter_on_the_orcagrid
  ...
  call get_command_argument(7, arg); read(arg, *) il_nbands

  IF (rank==0) THEN
     print *, "Input file:   ", trim(infile)
     print *, "Output file:  ", trim(outfile)
     print *, "Variable:     ", trim(varname)
     print *, "lmin/lmax/il_nbands:     ", lmin, lmax, il_nbands
  ENDIF

  ! allocate some work arrays
  allocate(ila_lmin(il_nbands))
  allocate(ila_lmax(il_nbands))
  ...
  ! Allocate work array for back proj
  ALLOCATE(rla_proj_nband(array_shape(1), array_shape(2), il_nbands))
  !Define the class of bands
  il_delta_nbands=(lmax-lmin)/il_nbands
  il_min=lmin
  il_max=il_min+il_delta_nbands
  DO il_ji=1,il_nbands
     ranfield_1d = 0.
     IF (il_ji==il_nbands) il_max=lmax
     PRINT*,'il_min',il_min,'il_max',il_max
     ila_lmin(il_ji) = il_min
     ila_lmax(il_ji) = il_max
     call back_ylm_loc(spectrum,ranfield_1d,x_1d,y_1d,il_min,il_max)
     rla_proj_nband(:,:,il_ji) = RESHAPE(ranfield_1d,(/SHAPE(area)/)) * mask
     il_min=il_max+1
     il_max=il_min+ il_delta_nbands -1
  ENDDO
  ...
  IF (rank==0) THEN  
     ...
     is = NF90_DEF_DIM(ncid,'z',il_nbands,idz)
     is = NF90_DEF_VAR(ncid,'lon',NF90_FLOAT,(/idx,idy/),idlon)
     is = NF90_DEF_VAR(ncid,'lat',NF90_FLOAT,(/idx,idy/),idlat)
     is = NF90_DEF_VAR(ncid,'lmin',NF90_FLOAT,(/idz/),idlmin)
     is = NF90_DEF_VAR(ncid,'lmax',NF90_FLOAT,(/idz/),idlmax)
     is = NF90_DEF_VAR(ncid,'ranfield',NF90_FLOAT,(/idx,idy, idz/),idv)
     is = NF90_ENDDEF(ncid)
     is = NF90_PUT_VAR(ncid,idlon,lon)
     is = NF90_PUT_VAR(ncid,idlat,lat)
     is = NF90_PUT_VAR(ncid,idv,rla_proj_nband)
     is = NF90_CLOSE(ncid)
  ENDIF
